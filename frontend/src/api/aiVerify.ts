 ...
 *
 * Scoped under a run. Three entry points (single-row, selection,
 * all-filtered) all hit the same start-stream endpoint with a
 * different ``match_ids`` array. The browser opens the SSE stream
 * with fetch + ReadableStream so the POST body carries the
...
/** Open the SSE stream for one verification session.
 *
...
export const AiVerify = {
  listActions: (runId: string, scopeKind: ScopeKind) =>
    api.get<AgentActionMeta[]>(
      `/runs/${runId}/ai-verify/actions?scope_kind=${scopeKind}`,
    ),

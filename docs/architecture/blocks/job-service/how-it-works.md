...
| `ner_verify` / `authority_verify` / `wikidata_verify` | `verify_job.py` | Opens the corresponding eval-agent event stream, tracks judged/total, embeds `session_snapshot` in `result` |

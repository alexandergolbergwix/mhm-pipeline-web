...
| `backend/tests/test_hmo_studio_ttl_restore.py` | `hmo_manifest_build` / `hmo_item_build` enqueue when TTL is Postgres-only |
| `backend/tests/test_hmo_studio_upload_manifests_router.py` | `hmo_manifest_upload` enqueue + worker (Rule W-107) |
| `backend/tests/test_hmo_studio_upload_items_router.py` | `hmo_item_upload` dry-run + live enqueue (Rule W-107) + `local_ids` job params |
...
| `backend/tests/unit/test_verify_job_progress.py` | Mid-run counters only; terminal slim snapshot (R20 / W-128) |
| `backend/tests/unit/test_verify_job_progress_throttle.py` | Slim snapshot strips evidence; mid-run omits snapshot (R20 / W-128) |

...
| `frontend/src/hooks/useVerifyJob.ts` | Verify job lifecycle (four verify kinds): poll/attach, upserts into `useRunJobs` for the global tray, hydrate from `progress.session_snapshot`, and roll back optimistic state when an enqueue request rejects |

...
22. **R22 — Interrupted verify jobs MUST be Continuable (Rule W-130).** `fail_stale_jobs` stamps `result.resumable` + judged/total for verify kinds; verify streams persist each verdict to cache/overrides incrementally; the UI Continues with `override_cache=false`. Job **list** responses omit `session_snapshot` (single-job GET may include it); resume probes use `?kind=&limit=`. *Why:* job 5bd42818 OOM'd at 61/313; opening Continue via full-history `/jobs` then R14'd again under Studio's 1.6 MB page load.

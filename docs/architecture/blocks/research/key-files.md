...
| `backend/app/services/research_agent/` | Grants, tool wrappers, wiki reads, `scope`/`sanitize`/`sparql_templates`/`title`/`pdf`, data skills (`data_info/select/distinct/search/agg`), movement-map + PDF skills, local AG-UI |

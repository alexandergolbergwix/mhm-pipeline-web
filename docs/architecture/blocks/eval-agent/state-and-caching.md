...
Wikidata item verdict payloads include `authority_evidence`, `work_candidate_evidence`, `local_reference_targets`, and the multi-channel `verify_evidence` pack (MARC + VIAF + Mazal + existing Wikidata + HMO Wikibase) alongside labels, statements, validation issues, and the exact MARC slice. `wikidata_verdict_query_summary` also fingerprints statement/property labels, qualifiers, and references. Both verify and merged-read paths attach local targets before computing `w124_v1` / `records_marc_v6` (Rule W-124; prior `w104_v1` / `w71_v1`), so changed evidence or skill/rubric contract invalidates stale cache entries without hiding a matching persisted verdict. Fingerprint projections remove nested transport metadata while retaining semantic date values (Rule W-208). Diagnostic exports expose the evidence as compact JSON for offline triage. HMO item salts use the same `w124_v1` bump.

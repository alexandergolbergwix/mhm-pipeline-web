...
- **R28 — Wikidata + HMO item judges MUST receive WikiProject Manuscripts skill context (Rule W-104 / W-124 / W-171 / W-172 / W-173).** Durable pack at `eval-agent/config/skills/wikidata_manuscripts/`; `skill_context_for(channel, entity_type, claim_pids)` injects always-rules, entity slices, claim triggers, and (HMO) the HMO→Wikidata projection checklist via `Evaluator.skill_context` → `render_prompt`. Never dump full wiki HTML. Cache salts `w173_v1` (prior `w172_v1` / `w171_v1`). Mode-β hygiene: absent claims ≠ defects; trust `value_label`; honour quantity **`unit` field**; never invent NLI for Cambridge `Q1028334`; facsimile semantic_type; do not invent disambiguation without evidence. *Why:* builders already followed WPM; judges did not; export-33/34 false partials from missing unit / invented holder labels.*

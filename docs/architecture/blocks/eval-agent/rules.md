...
- **R28 — Wikidata + HMO item judges MUST receive WikiProject Manuscripts skill context (Rule W-104 / W-124 / W-171).** Durable pack at `eval-agent/config/skills/wikidata_manuscripts/`; `skill_context_for(channel, entity_type, claim_pids)` injects always-rules, entity slices, claim triggers, and (HMO) the HMO→Wikidata projection checklist via `Evaluator.skill_context` → `render_prompt`. Never dump full wiki HTML. Cache salts `w171_v1` (prior `w124_v1`). Mode-β hygiene: absent claims ≠ defects; trust `value_label`; honour quantity units; facsimile semantic_type. *Why:* builders already followed WPM; judges did not, so HMO→public Wikidata readiness was under-evaluated; export-32 Mode-β partials needed universal rubric clauses.*

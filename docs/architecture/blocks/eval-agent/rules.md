...
- **R28 — Wikidata + HMO item judges MUST receive WikiProject Manuscripts skill context (Rule W-104 / W-124 / W-171 / W-172 / W-173 / W-174).** Durable pack at `eval-agent/config/skills/wikidata_manuscripts/`; `skill_context_for(channel, entity_type, claim_pids)` injects always-rules, entity slices, claim triggers, and (HMO) the HMO→Wikidata projection checklist via `Evaluator.skill_context` → `render_prompt`. Never dump full wiki HTML. Cache salts `w174_v1` (prior `w173_v1` / `w172_v1` / `w171_v1`). Mode-β hygiene: absent claims ≠ defects; trust `value_label` / holding-table glosses (Leeds `Q24568958`, Cambridge `Q1028334`); honour quantity **`unit` field**; Hebrew 245 as MS alias under designation label is intentional (W-164); catalogue P973 must agree with P217; never invent NLI/Robarts for audited holders; facsimile semantic_type; do not invent disambiguation without evidence. *Why:* builders already followed WPM; judges did not; export-33/35 false partials from missing unit / invented holder labels / cross-record 856.*

...
- **R28 — Wikidata + HMO item judges MUST receive WikiProject Manuscripts skill context (Rule W-104 / W-124).** Durable pack at `eval-agent/config/skills/wikidata_manuscripts/`; `skill_context_for(channel, entity_type, claim_pids)` injects always-rules, entity slices, claim triggers, and (HMO) the HMO→Wikidata projection checklist via `Evaluator.skill_context` → `render_prompt`. Never dump full wiki HTML. Cache salts `w124_v1`. *Why:* builders already followed WPM; judges did not, so HMO→public Wikidata readiness was under-evaluated.
...
- **R31 — Large-scope verify MUST stay alive under dyno pressure (Rule W-127).** OpenAI-compat spawn forces `--parallel 1`; judge HTTP waits emit mid-call `[STEP]` via `StepHeartbeat`; the judge loop appends `results.jsonl` per verdict; missing `runner.exit` synthesizes a `runner_error`; verify progress snapshots are throttled (see job-service R20). *Why:* job 06b44db0 stopped at 52/313 with no exit/checkpoint after DeepSeek parallel + megabyte progress writes starved the dyno.

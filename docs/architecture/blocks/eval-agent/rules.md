...
- **R28 — Wikidata + HMO item judges MUST receive WikiProject Manuscripts skill context (Rule W-104 / W-124 / W-171 / W-172 / W-173 / W-174 / W-175).** Durable pack at `eval-agent/config/skills/wikidata_manuscripts/`; `skill_context_for(channel, entity_type, claim_pids)` injects always-rules, entity slices, claim triggers, and (HMO) the HMO→Wikidata projection checklist via `Evaluator.skill_context` → `render_prompt`. Never dump full wiki HTML. Cache salts `w175_v1` (prior `w174_v1`…). Mode-β hygiene: absent claims ≠ defects; trust `value_label` / holding-table glosses; honour quantity **`unit` field**; Hebrew 245 as MS alias under designation label is intentional (W-164); catalogue P973 must agree with P217; sticky-full survives schema/gloss-only drift (W-175); never invent NLI/Robarts for audited holders. *Why:* builders already followed WPM; judges did not; salt bumps blanked pills until sanitise became sticky-full-aware.*
...
- **R39 — Judge transport and parse failures MUST use the abstain provider-error state (Rule W-207 / W-211).** The shared verify path persists `overall="abstain"`, `verification_status="provider_error"`, `judge_failure=true`, and `verification_error`; it never caches the failure as an `ai_verdict` inference result. The job still reports `partial`, and legacy `unknown` / `verification_failed` rows normalize on read. *Why:* nine run-48ba6c13 provider failures appeared as a separate `unknown` review bucket after a force rebuild.
- **R40 — Verify fingerprints MUST exclude volatile metadata but retain semantic dates (Rule W-208).** Build, canonical-entity, and AI-verdict projections remove nested transport timestamps, request IDs, and fetch times. They retain semantic manuscript dates and Wikidata time claims. *Why:* metadata changed SHA values for unchanged content.
...
- **R44 — Jev (typesafe provider) verdicts keep the certified harness behavior.** `TypesafeJudge` rewrites only the trailing "Return only the JSON verdict." line of the tier-1 prompt (byte parity is what the bake-off certified); the tuned question sets in `typesafe_questions.py` are ported verbatim and never regenerated from schemas; the `overall` is computed in code from the universal table (never asked of the model); the deterministic gates (`jev_gates.py` — artifact downgrade, ERROR-severity validator gate, `ROLE_CONF_GATE`) run in the session with the candidate payload, not in the client and not inside model questions; raw Jev answers + judgment confidence travel in `JudgeResponse.meta`, so cached verdicts stay schema-clean under `verdict.v2.json` (`additionalProperties: false`); escalation (`--escalate-policy`, typesafe-only, default OFF) records the deciding judge in `judge_id`, and cache-hit rows re-check the fallback on overall alone. **Wikidata-contract extension (2026-09-21):** for `wikidata_item` the set gains two questions — `p31_ok` (instance-of typing per the WPM data model) and `duplicate_risk` (existing-QID / live-probe status) — and three mechanical gates that the confidence gate can never soften: a live duplicate probe of `candidates_found` without an adopted existing QID forces `type_ok=no` (an identifier match is an identity match — Rule W-139), an item with no P31 statement at all forces `type_ok=no` (structural claim missing), and the `duplicate_risk` answer participates in the overall (duplicate_found blocking; unknown may not move any axis). The added questions extend the certified set — re-run the wikidata-channel bake-off before defaulting that channel to Jev. *Why:* the certification measured exactly this pipeline — any drift (regenerated question wording, gates in the model prompt, extra verdict fields, schema-invalid cache rows) silently invalidates the measured accuracy/unsafe numbers; and a CREATE against a probed identifier match is the duplicate failure mode the April 2026 bulk-deletion request was about.

- **R45 — A verify pre-spawn failure surfaces its real cause (Rule W-258).** The verify stream (`hmo_item_verify.py` and every channel stream) catches exceptions from the spawn phase — missing judge credentials (`Tier1CredentialsError`), unresolvable model, fixture writer failure — and carries the message into `runner_error` / `session.end`; the never-ran outcome may not be masked by the vague "stopped after 0 of N verdicts" synthesis, and the job tray must not read "Verification complete" on a partial outcome. *Why:* run f4e8e4b3 (2026-09-22) picked `typesafe/jev-1.13.0`, the `mhm-jobs2` Modal secret carried no `TYPESAFE_API_KEY`, `ensure_tier1_credentials` raised inside `spawn_eval_agent_run` before any subprocess existed — and both verify jobs still finalized `succeeded` with "Verification complete", 0 of 2705 judged.

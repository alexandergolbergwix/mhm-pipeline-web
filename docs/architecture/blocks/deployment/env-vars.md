...
| `MODAL_JOBS_URL`, `MODAL_JOBS_TOKEN` | `modal_job_client.py`, `modal/modal_jobs.py` | Modal execution of `rdf_build` / `hmo_item_build` (Rule W-237): empty URL → Heroku-local runner; the token must match the `mhm-jobs` Modal secret (which also carries `DATABASE_URL`) |

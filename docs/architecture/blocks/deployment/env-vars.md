...
| `RUN_JOB_CANCEL_FORCE_AFTER_S` | `run_job_service.py` | Cancel grace for running jobs (default 300 s, 0 disables): the maintenance tick force-finalizes any running row whose `cancel_requested_at` is older — the executor wedged in a non-polling stretch or died (Rule W-236) |
...
| `RULE_VERIFY_WD_PROBE_MAX`, `RULE_VERIFY_WD_QID_MAX` | `rule_verify_job.py`, `rule_verify/rules/hmo.py` | Probe/QID budgets for the Heroku fallback's inline API checks per 1 000-item chunk (defaults 300 / 2 000). Exhausted budgets abstain `not_relevant` — never a silent pass (R56, W-255) |
| `RULE_VERIFY_API_PASS_PROBE_MAX`, `RULE_VERIFY_API_PASS_QID_MAX` | `rule_verify_job.py` (`_api_pass_budgets`) | Whole-scope budgets for the single-container Modal API pass (Rule W-256; defaults 30 000 / 60 000 — ~30 k probes × ~1.15 s ≈ 9.5 h worst case, so raise the 6 h container timeout in `modal/modal_jobs.py` together) |
...
| `GEMINI_API_KEY`, `QUBRID_API_KEY`, `TYPESAFE_API_KEY`, `HF_*_REPO`, `MHM_MODEL_DIR`, `KIMA_DB_PATH`, `MAZAL_DB_PATH`, `DISABLE_VIAF/KIMA/WIKIDATA`, `MHM_NO_NETWORK`, `DISABLE_PG_LISTENER` | pipeline modules | LLM judge keys (Gemini default tier-1; Qubrid for Kimi K2.5 + DeepSeek V4 Flash — Rule W-46; TypeSafe for the Jev `typesafe/jev-1.13.0` tier-1 judge — R44), model/data locations and kill switches |

...
| `backend/app/pipeline/hmo_item_upload.py` | Two-pass upload: pass 1 create/adopt/update items, pass 2 deferred item→item links |
...
| `backend/app/routers/hmo_studio_items.py` | Per-item review API: list, override PATCH, reconcile, export/import, AI verify, AI-fix apply |
...
| `backend/converter/wikibase/hmo_exporter.py` | TTL → entity drafts; label/description truncation (250) and string claim truncation (400) |
...
| `backend/app/services/wikibase_audit.py` | `record_wikibase_write` — one `wikibase_cloud_writes` row per outcome; never raises |
...
| `frontend/src/components/hmo/*` | Review UI: `HmoItemTable`, `HmoItemDetailDrawer`, `HmoItemShaclBadge`, verification modals, schema panels |

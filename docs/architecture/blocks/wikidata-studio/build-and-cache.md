...
`viaf_uri`/`mazal_id`/`wikidata_qid` — Rule W-188). Then
`merge_legacy_into_canonical` omits persons unless the item has an existing
QID or a publishable P214/P8189/P244/P227/P213/P268 statement (Rule W-154).
The canonical assembler repeats recover-then-omit after curator overrides
and reconciliation, immediately before validation and cache persistence.

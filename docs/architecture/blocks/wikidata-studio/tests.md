...
- `backend/tests/test_wikidata_item_views.py` — merged view validates verdicts against the retained pre-derived override projection (R66).
- `backend/tests/test_wikidata_item_views.py` — merged view applies the persist-slim projection before stable-key validation (R67) and retains subset verdicts when local-target labels differ by scope (R68).

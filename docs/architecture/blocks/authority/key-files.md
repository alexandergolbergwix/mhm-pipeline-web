...
| `backend/app/pipeline/authority_re_enrich.py` | Retained only for migration/rollback compatibility; standalone callers are retired by default |
| `backend/app/pipeline/authority_re_enrich_job.py` | Legacy job implementation; job creation/execution is fail-closed unless the emergency rollback flag is enabled |
